...
export class Usuarios implements OnInit, OnDestroy {
  ...
  error = '';
  success = '';
  filtroActivo: 'todos' | 'admins' | 'activos' | 'admins-activos' = 'todos'; // Filtro usando Specification Pattern
  ...
  /**
   * Carga solo administradores usando AdminUsuarioSpecification
   */
  cargarAdministradores(): void {
    this.usuarioPatronesService.listarAdministradores().subscribe({
      next: (usuarios) => {
        this.usuarios = [...usuarios];
        this.aplicarPaginacion();
        this.loading = false;
  ...
  /**
   * Carga solo usuarios activos usando ActiveUsuarioSpecification
   */
  cargarUsuariosActivos(): void {
    this.usuarioPatronesService.listarUsuariosActivos().subscribe({
      next: (usuarios) => {
        this.usuarios = [...usuarios];
        this.aplicarPaginacion();
        this.loading = false;
  ...
  /**
   * Carga administradores activos usando Composite Specification Pattern
   */
  cargarAdministradoresActivos(): void {
    this.usuarioPatronesService.listarAdministradoresActivos().subscribe({
      next: (usuarios) => {
        this.usuarios = [...usuarios];
        this.aplicarPaginacion();
        this.loading = false;
  ...
  /**
   * Carga administradores sin indicador de loading
   */
  ...
  /**
   * Carga usuarios activos sin indicador de loading
   */
  ...
  /**
   * Carga administradores activos sin indicador de loading
   */
    ...
    if (confirm(`¿Desea inactivar el usuario "${email}"?\n\nEl usuario no será eliminado, solo marcado como inactivo.`)) {
      ...
      this.usuarioService.inactivarUsuario(id).subscribe({
        next: (response) => {
          ...
          this.usuarios = this.usuarios.map((u: UsuarioDTO) => {
            if (u.id === id) {
  ...
  private setupFormValidators(): void {
    // Para crear nuevo usuario, contraseña es requerida
  ...
  private setupFormValidatorsForEdit(): void {
    ...
    this.usuarioForm.get('name')?.setValidators([Validators.required, Validators.minLength(2)]);
