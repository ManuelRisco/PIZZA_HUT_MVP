...
export class Pizzas implements OnInit {
  ...
  aplicarFiltro(filtro: 'todas' | 'disponibles' | 'populares' | 'disponibles-populares'): void {
    ...
    observable.subscribe({
      ...
      error: (error) => {
        console.error('❌ Error al filtrar pizzas:', error);
        this.mostrarMensajeError('Error al aplicar filtro');
        this.loading = false;
      }
    });
  }
   ...
   * Usado después de crear/editar para mantener el filtro activo
   */
